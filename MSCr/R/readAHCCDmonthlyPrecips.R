...
#' @author Kevin Shook
#' @seealso  \code{\link{readAHCCDdailyTemps}} \code{\link{readAHCCDdailyPrecips}}
...
#' @examples
#' \dontrun{
#' stoon.monthly.total <- readAHCCDmonthlyPrecips('mt4057120.txt')}
#' @export
...
readAHCCDmonthlyPrecips <- function(infile) {
  ...
  if (stringr::str_detect(stringr::str_to_lower(filename), "mt")) {
    precip.type <- "total"
  }
  else if (stringr::str_detect(stringr::str_to_lower(filename), "mr")) {
    precip.type <- "rain"
  }
  else if (stringr::str_detect(stringr::str_to_lower(filename), "ms")) {
    precip.type <- "snow"
  }
  else {
  ...
  # read data
  raw <- utils::read.csv(file = infile, header = FALSE, skip = 4)
  row.count <- nrow(raw)
  # now unstack data
  data.row <- 0
