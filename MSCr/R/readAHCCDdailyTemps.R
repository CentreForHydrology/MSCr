...
#' @author Kevin Shook
#' @seealso  \code{\link{readAHCCDdailyTemps}} \code{\link{readAHCCDmonthlyPrecips}} 
...
readAHCCDdailyTemps <- 
  function(infile){
    ...
    split <- stringr::str_split_fixed(base, fixed('.'), 2)
    filename <- split[1]
  ...
  input <- tolower(input)
  LineNum <- stringr::str_detect(input, fixed(','))
  fileHeaderLines <- sum(LineNum)
  
  # now find number of lines containing column titles
  englishLineNum <- stringr::str_detect(input, fixed("year"))
  englishLineCount <- sum(englishLineNum)
  frenchLineNum <- stringr::str_detect(input, fixed("annee"))
  ...
  # read data
  raw <- utils::read.fwf(file=infile, widths=all, header=FALSE, colClasses=all.classes, skip=totalSkipLines)
  row.count <- nrow(raw)
  # now unstack data
  data.row <- 0
