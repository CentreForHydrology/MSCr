...
#' @param secondaryName Required. The name of the secondary CRHM dataframe.
...
downloadMSCobs <-
function(station.name='', station.number='', startyear=1900, endyear=2000, quiet=FALSE, logfile=''){
  ...
  if((hourly.t.length==0) & (hourly.rh.length == 0) & (hourly.u.length == 0))
   ...
   createHourlyObsFile(hourly, ObsFile, station.name, station.number, quiet)
 ...
 # delete .csv files
  ...
  createDailyTempObsFile(daily, ObsFile, station.name, station.number, quiet)

  ObsFile <- paste(folder,'/',station.name,'DailyPrecips.obs', sep='')
  createDailyPrecipObsFile(daily, ObsFile, station.name, station.number, quiet)
