...
#'
#' @description Reads large MSC files holding daily values of several variables at several sites and exports an hourly CRHM \code{obs} data file for each site. The obs files are of the form \option{<siten umber>_daily.obs'}
...
BigMSCDailyToObs <- function(infile, quiet=TRUE, logfile=''){
  ...
  for (station.num in 1:stations.count){
    ...
    last.month.start.date <- as.Date(paste(station.data[last.row,2],'-',station.data[last.row,3],
                                           '-01', sep=''), format='%Y-%m-%d')
    last.month.days <- days_in_month(last.month.start.date)
    ...
    if (exists('obs')){
      ...
      obs.name <- paste(each.station, '_daily.obs', sep='')
      result <- writeObsFile(obs, obs.name, quiet, logfile)
      rm(obs)
    }
  }
}
