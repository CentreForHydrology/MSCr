...
#' @description Reads very old Atmospheric Environment Service (AES) files. The files are named \code{WINDXX}, where XX is the last 2 digits of the year. All of the files are assembled to create a \code{CRHM} observation file named \option{u.obs} in the same directory as the data files. The wind speeds are converted from km/h to m/s.
