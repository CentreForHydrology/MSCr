...
#' @examples
#' \dontrun{
#'stoon.snowfall <- readAHCCDdailyPrecips('3031093_S.txt')}
#' @export
...
readAHCCDdailyPrecips <- 
  ...
  englishLineNum <- stringr::str_detect(input, stringr::fixed("year"))
  englishLineCount <- sum(englishLineNum)
  frenchLineNum <- stringr::str_detect(input, stringr::fixed("annee"))
  ...
  # read data
  raw <- utils::read.fwf(file=infile, widths=all, header=FALSE, 
                         colClasses=all.classes, skip=totalSkipLines)
  row.count <- nrow(raw)
  # now unstack data
  data.row <- 0
