#' Creates an obs file from all MSC old-style temperature files in a directory 
#' @description  Reads all files of old-style MSC hourly temperature data from a specified directory and assembles them. The values are organized day x hour, 1 year per file. Writes an obs file called \option{t.obs}.
#' @param directory Optional. Directory containing all temperature files. File names must begin with \code{TE}. The default is the current directory.
...
#' @examples \dontrun{
#' 
#' oldMSChourlyTtoObs(timezone='Etc/GMT+7')
#' }
oldMSChourlyTtoObs <- function(directory='.', timezone){
