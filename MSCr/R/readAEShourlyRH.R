#' Reads individual AES files of hourly RH data in a directory and creates obs files.
#' @description Reads very old Atmospheric Environment Service (AES) files. The files are named \code{RHXX}, where XX is the last 2 digits of the year. All of the files are assembled to create a \code{CRHM} observation file named \option{rh.obs} in the same directory as the data files.
...
#' @author Kevin Shook
#' @seealso  \code{\link{readAEShourlyWind}} \code{\link{readAEShourlyT}} code{\link{readAESdailyTminTmax}} \code{\link{readAESdailyP}}
#' @export
#'
#' @examples
#' \dontrun{
#' readAEShourlyRH('./HistoricalData', timezone='etc/GMT+6')
#' }
readAEShourlyRH <- function(directory='.', timezone=''){
